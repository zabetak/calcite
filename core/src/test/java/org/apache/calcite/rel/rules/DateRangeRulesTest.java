...
class DateRangeRulesTest {
  ...
  private static class Fixture2 extends Fixture {
    ...
    Fixture2() {
      ...
      exMonthD = rexBuilder.makeCall(
          SqlStdOperatorTable.EXTRACT,
          ImmutableList.of(rexBuilder.makeFlag(TimeUnitRange.MONTH), d));
      exDayD = rexBuilder.makeCall(
          SqlStdOperatorTable.EXTRACT,
          ImmutableList.of(rexBuilder.makeFlag(TimeUnitRange.DAY), d));

      floorYear = rexBuilder.makeCall(SqlStdOperatorTable.FLOOR,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.YEAR)));
      floorMonth = rexBuilder.makeCall(SqlStdOperatorTable.FLOOR,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.MONTH)));
      floorDay = rexBuilder.makeCall(SqlStdOperatorTable.FLOOR,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.DAY)));
      floorHour = rexBuilder.makeCall(SqlStdOperatorTable.FLOOR,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.HOUR)));
      floorMinute = rexBuilder.makeCall(SqlStdOperatorTable.FLOOR,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.MINUTE)));

      ceilYear = rexBuilder.makeCall(SqlStdOperatorTable.CEIL,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.YEAR)));
      ceilMonth = rexBuilder.makeCall(SqlStdOperatorTable.CEIL,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.MONTH)));
      ceilDay = rexBuilder.makeCall(SqlStdOperatorTable.CEIL,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.DAY)));
      ceilHour = rexBuilder.makeCall(SqlStdOperatorTable.CEIL,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.HOUR)));
      ceilMinute = rexBuilder.makeCall(SqlStdOperatorTable.CEIL,
          ImmutableList.of(ts, rexBuilder.makeFlag(TimeUnitRange.MINUTE)));
    }
  }
}
